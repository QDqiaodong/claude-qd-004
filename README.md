...
**淘汰按会计规矩走专门的一笔账**（`POST /api/cows/{id}/cull`，档案页【登记淘汰】）：
同一事务内把泌乳状态置 `已淘汰`、在栏状态置 `离栏`、并把牛舍名额摘掉（`barn_id` 置空），三件事一起落库；
只改状态不腾名额、或只腾名额还挂着泌乳中都过不去，事务中途断开整笔回滚，不会出现淘汰了还占着名额的账。
普通保存（`PUT`）里不允许把泌乳状态就地改成「已淘汰」，图省事只改状态的做法会被拦下。
淘汰即封档：这头牛不能再分回任何一间牛舍、不能改回泌乳/在栏（只留昵称、品种可改）；
耳号有全库唯一约束，淘汰耳号也不能再拿来新建占名额的档案。牛舍「在栏头数」实时按档案统计，淘汰后名额立刻腾出。

- 页面：奶牛档案（`/cows`）
- 接口：`GET/POST /api/cows`、`PUT /api/cows/{id}`、`POST /api/cows/{id}/cull`
